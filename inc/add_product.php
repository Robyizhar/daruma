...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        if (!empty($name) && !empty($description) && !empty($edition) && $price > 0 && !empty($category)) {
            /* Handle file upload */
            ...
            $Model = new Model();
            $query = $Model->addProduct($name, $category, $description, $edition, $price, $imagePath); /* Insert produk in to database */
