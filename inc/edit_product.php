...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if ($id > 0 && !empty($name) && !empty($description) && !empty($edition) 
            ...
            // Update product with the new stock field
            $query = $Model->updateProduct($id, $name, $category, $description, $edition, $price, $stock, $imagePath);
            ...
            if ($query) {
                ...
                $response["message"] = "Error updating product in database.";
                echo json_encode($response);
            }
